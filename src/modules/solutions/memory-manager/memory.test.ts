...
test('alloc выбрасывает ошибку, если в куче не хватает памяти', () => {
  const memory = createMemory(100, { stack: 10 });
...
test('change записывает данные в выделенный блок кучи', () => {
  const memory = createMemory(100, { stack: 10 });

  const pointer = memory.alloc(3);

  pointer.change(new Uint8Array([7, 8, 9]).buffer);

  assert.deepEqual(bytes(pointer.deref()), [7, 8, 9]);
  assert.deepEqual(bytes(memory.buffer).slice(16, 19), [7, 8, 9]);
});

test('free выбрасывает ошибку для указателя из стека', () => {
  const memory = createMemory(100, { stack: 10 });
...
test('free освобождает указатель из кучи и делает его недействительным', () => {
  const memory = createMemory(100, { stack: 10 });
...
test('alloc переиспользует освобождённый блок кучи', () => {
  const memory = createMemory(100, { stack: 10 });

  const p1 = memory.alloc(20);

  p1.free();

  const p2 = memory.alloc(10);

  assert.equal(p2.offset, p1.offset);
  assert.equal(p2.size, 10);
});

test('free объединяет соседние свободные блоки кучи', () => {
  const memory = createMemory(100, { stack: 10 });

  const p1 = memory.alloc(10);
  const p2 = memory.alloc(10);

  p1.free();
  p2.free();

  const p3 = memory.alloc(20);

  assert.equal(p3.offset, 16);
  assert.equal(p3.size, 20);
});
...
test('free объединяет соседние блоки независимо от порядка освобождения', () => {
  const memory = createMemory(100, { stack: 10 });

/*! 
Генератор JSON файлов для теста

Использование:
  node generate-json.mjs [опции]

Опции:
  --rows, -n <число>        Количество объектов (по умолчанию 100)
  --output, -o <файл>       Имя выходного файла (по умолчанию data.json)
  --format, -f <тип>        Формат: json (массив) или jsonl (JSON Lines)
...
Примеры:
  # Только генерация
  node generate-json.js -n 1000 -o users.json

  # Генерация + Brotli сжатие
  node generate-json.js -n 10000 -f jsonl -c br

  # Генерация + ZIP и Brotli
  node generate-json.js -n 50000 -c zip,br

  # Сжать всеми методами
  node generate-json.js -n 1000000 --compress-all

  # С указанием формата и сжатия
  node generate-json.js -n 100000 -f jsonl -o data.jsonl -c zip,br,gz
...
Требования:
  ...
  Brotli и Gzip работают без дополнительных зависимостей
    ...
    if (!output.includes(".")) {
        output = format === "jsonl" ? `${output}.jsonl` : `${output}.json`;
    }

    console.log(`\n📝 Параметры: ${count.toLocaleString()} объектов → ${output} (формат: ${format})`);
    ...
    let result;
    if (format === "jsonl") {
        result = await generateJsonlStream(count, output);

    } else {
        result = await generateJsonStream(count, output);
    }
...
function generateJsonObject(id) {
...
async function generateJsonStream(rowsCount = 100, outputFile = "data.json") {
    console.log(`🚀  Генерация ${rowsCount.toLocaleString()} объектов в потоковом режиме...`);
    console.log(`📝  Формат: JSON массив (валидный)`);
    ...
    function writeObject(jsonString, isLast) {
        return new Promise((resolve) => {
            const comma = isLast ? "" : ",";
            const canWrite = writeStream.write(jsonString + comma + "\n");
    ...
    for (let i = 1; i <= rowsCount; i++) {
        const obj = generateJsonObject(i);
        const jsonString = JSON.stringify(obj, null, 2);
        const isLast = (i === rowsCount);

        await writeObject(jsonString, isLast);
        rowCount++;
        ...
        if (progress >= lastProgress + 1 || i % 10000 === 0) {
            ...
            process.stdout.write(`\r📊  Прогресс: ${progress}% (${i.toLocaleString()}/${rowsCount.toLocaleString()}) • ${rate.toLocaleString()} объектов/сек`);
        }
    }
    ...
    return new Promise((resolve, reject) => {
        writeStream.on("finish", () => {
            ...
            console.log(`\n✅ JSON сохранён в ${outputFile}`);
            console.log(`📁  Размер: ${fileSize} MB`);
            console.log(`⏱️  Время: ${totalTime} сек`);
            console.log(`📈  Средняя скорость: ${Math.round(rowCount / totalTime).toLocaleString()} объектов/сек`);
...
async function generateJsonlStream(rowsCount = 100, outputFile = "data.jsonl") {
    console.log(`🚀  Генерация ${rowsCount.toLocaleString()} объектов в потоковом режиме...`);
    console.log(`📝  Формат: JSONL (каждая строка - отдельный JSON объект)`);
    ...
    function writeObject(jsonString) {
        return new Promise((resolve) => {
            const canWrite = writeStream.write(jsonString + "\n");

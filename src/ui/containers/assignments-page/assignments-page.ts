  ...
  private async loadAssignment(item: Assignment): Promise<void> {
    ...
    if (item.id === 'hw-05') {
      ...
      this.slots.solutionTldrEl.innerHTML =
        ...
        '<li>Для объектов и функций используется identity-хэш через скрытый <code>Symbol</code>. Поэтому один и тот же объект остаётся тем же ключом, а разные объекты с одинаковым содержимым считаются разными ключами.</li>' +
